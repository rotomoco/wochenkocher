...
function generateWeekDays(date: Date): WeekDay[] {
  const weekStart = startOfWeek(date, { weekStartsOn: 1 }); // Montag als erster Tag
...
export default function WeekplanningView() {
  ...
  const [weekDays, setWeekDays] = useState<WeekDay[]>(() => generateWeekDays(new Date()));
  ...
  async function loadWeekPlan() {
    try {
      ...
      // Lade geplante Gerichte für die Woche
      ...
      // Füge die geplanten Gerichte den entsprechenden Tagen hinzu
  ...
  async function generateRandomMeals() {
    try {
      ...
      // Hole 7 zufällige Gerichte
      const randomDishes = await getRandomDishes(7);
      
      // Aktualisiere den Wochenplan
